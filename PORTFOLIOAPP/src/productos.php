...
if ($order == 0) {
  ...
  elseif ($order == 2) {
  $productos = orden_por_fecha($productos, 2);
} elseif ($order == 3) {
  $productos = orden_por_fecha($productos, 3);
}
?>

<!-- Filtrado de productos segun categoría -->
<?php 
...
<!-- Botones de orden y dejar de filtrar -->
<div class="d-flex mx-auto div_botones">
...
<!-- "Página" de productos no encontrados -->
...
<!-- Creación de fichas de productos -->
  <div class="row">
    <?php $prod_pag = ver_productos($page,$prod_por_pag);
          for($i = $prod_pag[0];$i < $prod_pag[1]; $i++):
          if ($productos[$i]['id']) {?>
      <div class="col-sm-3">
  ...
  <!-- desplegable selector de cuantos productos aparecerán por página -->
  <div class="div_ppp">
